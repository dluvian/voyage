...
import com.dluvian.voyage.navigator.BookmarksNavView
import com.dluvian.voyage.navigator.CreateGitIssueNavView
import com.dluvian.voyage.navigator.CreatePostNavView
import com.dluvian.voyage.navigator.CrossPostCreationNavView
...
) : VoteEvent(postId = ClickUpvote.postId, mention = ClickUpvote.mention)

data class ClickNeutralizeVote(
    override val postId: EventIdHex,
    override val mention: PubkeyHex,
) : VoteEvent(postId = ClickNeutralizeVote.postId, mention = ClickNeutralizeVote.mention)

sealed class TopicEvent(open val topic: Topic) : UIEvent()
data class FollowTopic(override val topic: Topic) : TopicEvent(topic = FollowTopic.topic)
data class UnfollowTopic(override val topic: Topic) : TopicEvent(topic = UnfollowTopic.topic)


sealed class BookmarkEvent(open val postId: EventIdHex) : UIEvent()
data class BookmarkPost(override val postId: EventIdHex) :
    BookmarkEvent(postId = BookmarkPost.postId)

data class UnbookmarkPost(override val postId: EventIdHex) :
    BookmarkEvent(postId = UnbookmarkPost.postId)


sealed class ProfileEvent(open val pubkey: PubkeyHex) : UIEvent()

data class FollowProfile(override val pubkey: PubkeyHex) :
    ProfileEvent(pubkey = FollowProfile.pubkey)

data class UnfollowProfile(override val pubkey: PubkeyHex) :
    ProfileEvent(pubkey = UnfollowProfile.pubkey)
